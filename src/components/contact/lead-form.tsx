  ...
  if (status === "success") {
    return (
      <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-8 text-center">
        ...
        <p className="mt-4 text-xs text-gray-400">提交编号：{submissionId.slice(0, 8)}</p>
      </div>
    );
  }

  const inputClass = "w-full rounded-lg border border-gray-200 px-4 py-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500";
      ...
      {tab === "employer" ? (
        <form onSubmit={employerForm.handleSubmit(onEmployerSubmit)} className="space-y-5">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              ...
              <input {...employerForm.register("name", { required: true, minLength: 2, maxLength: 20 })} className={inputClass} placeholder="您的姓名" />
              {employerForm.formState.errors.name && <p className={errorClass}>请输入 2-20 字的姓名</p>}
            </div>
            <div>
              <label className={labelClass}>手机号 *</label>
              <input {...employerForm.register("phone", { required: true, pattern: /^1[3-9]\d{9}$/ })} className={inputClass} placeholder="11 位手机号" />
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              ...
              <input {...employerForm.register("company", { required: true, minLength: 2, maxLength: 50 })} className={inputClass} placeholder="公司全称" />
              {employerForm.formState.errors.company && <p className={errorClass}>请输入公司名称</p>}
            </div>
            <div>
              <label className={labelClass}>职位 *</label>
              <input {...employerForm.register("position", { required: true, minLength: 2, maxLength: 30 })} className={inputClass} placeholder="您的职位" />
              {employerForm.formState.errors.position && <p className={errorClass}>请输入职位</p>}
            </div>
          </div>
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              <label className={labelClass}>月均招聘职位数 *</label>
              <select {...employerForm.register("monthlyPositions", { required: true })} className={inputClass}>
            ...
            <div>
              <label className={labelClass}>主要招聘城市 *</label>
              <input {...employerForm.register("primaryCity", { required: true, minLength: 2, maxLength: 30 })} className={inputClass} placeholder="如：北京、上海" />
              {employerForm.formState.errors.primaryCity && <p className={errorClass}>请输入城市</p>}
            </div>
          </div>
          <div>
            <label className={labelClass}>目前使用的招聘工具</label>
            <div className="flex flex-wrap gap-3">
              {toolOptions.map((tool) => (
                <label key={tool} className="flex items-center gap-2 text-sm text-gray-600">
        ...
        <form onSubmit={candidateForm.handleSubmit(onCandidateSubmit)} className="space-y-5">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              <label className={labelClass}>姓名 *</label>
              <input {...candidateForm.register("name", { required: true, minLength: 2, maxLength: 20 })} className={inputClass} placeholder="您的姓名" />
              {candidateForm.formState.errors.name && <p className={errorClass}>请输入姓名</p>}
            </div>
            <div>
              <label className={labelClass}>手机号 *</label>
              <input {...candidateForm.register("phone", { required: true, pattern: /^1[3-9]\d{9}$/ })} className={inputClass} placeholder="11 位手机号" />
              {candidateForm.formState.errors.phone && <p className={errorClass}>请输入有效的手机号</p>}
            </div>
          </div>
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              <label className={labelClass}>目标岗位 *</label>
              <input {...candidateForm.register("targetPosition", { required: true, minLength: 2, maxLength: 30 })} className={inputClass} placeholder="如：前端工程师" />
              {candidateForm.formState.errors.targetPosition && <p className={errorClass}>请输入目标岗位</p>}
            </div>
            <div>
              <label className={labelClass}>目标城市 *</label>
              <input {...candidateForm.register("targetCity", { required: true, minLength: 2, maxLength: 20 })} className={inputClass} placeholder="如：北京" />
